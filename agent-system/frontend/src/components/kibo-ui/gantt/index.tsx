...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({
  ...
  return (
    ...
    >
      ...
      {showDuration && <p className="pointer-events-none text-muted-foreground">{duration}</p>}
    </div>
  );
};
...
export const GanttSidebarHeader: FC<GanttSidebarHeaderProps> = ({
  ...
  >
    ...
    {subtitle && <p className="shrink-0">{subtitle}</p>}
  </div>
);
...
export const GanttSidebar: FC<GanttSidebarProps> = ({
  ...
  >
    <GanttSidebarHeader title={headerTitle} subtitle={headerSubtitle} />
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  // Scroll to today on mount (instead of center of total width)
  useEffect(() => {
    if (scrollRef.current) {
      const today = new Date();
      const timelineStart = new Date(timelineData[0]?.year ?? today.getFullYear(), 0, 1);
      const ctx: GanttContextProps = {
        zoom, range, columnWidth, sidebarWidth, headerHeight, rowHeight,
        onAddItem, placeholderLength: 2, timelineData, ref: scrollRef,
      };
      const todayOffset = getOffset(today, timelineStart, ctx);
      // Position today ~60% from left edge so bars are visible on the left and future on the right
      const targetScroll = Math.max(0, todayOffset - scrollRef.current.clientWidth * 0.6);
      scrollRef.current.scrollLeft = targetScroll;
      setScrollX(scrollRef.current.scrollLeft);
    }
  }, [setScrollX, timelineData, zoom, range, columnWidth, sidebarWidth, onAddItem]);
